  ...
  if (window.innerWidth >= 360) {
    ...
    if (worksSection) {
      ...
      const cardWrappers = document.querySelectorAll(
        ".nav-pages .card-wrapper, .nav-about-page .card-wrapper"
      );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  toolsToggles.forEach((toggle) => {
    toggle.addEventListener("click", function () {
      ...
      document
        .querySelectorAll(".tools-toggle.active")
        .forEach((activeToggle) => {
          if (activeToggle !== this) {
            activeToggle.classList.remove("active");
            activeToggle.querySelector(".tools-dropdown").style.display =
              "none";
            activeToggle.querySelector(".crab").style.transform =
              "rotate(0deg)";
          }
        });
